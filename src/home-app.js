...
function init(){
  ...
  /* The dropzone is role="region" tabindex="0", so a keyboard user tabs
     onto the biggest target on the page, gets a focus ring, and finds
     that Enter and Space do nothing — the real control is the file
     input on the very next stop. That made the region a dead stop
     immediately before the thing it looks like it should trigger.
     Forward the two activation keys to the input. Only when the region
     itself has focus: once focus moves to the input, the browser's own
     handling takes over and we must not double-fire. */
  const dzKeyTarget = document.getElementById('dropzone');
  if (dzKeyTarget && fi) {
    dzKeyTarget.addEventListener('keydown', (e) => {
      if (e.target !== dzKeyTarget) return;
...
/* R145 — lazy loader for the shared exotic decoders (TIFF/SVG/ICO). Only
   fetched when a file of those types is actually dropped, so the common
   JPG/PNG path pays nothing. */
let _exoticDecoderPromise = null;
function _ensureExoticDecoders(){
  if (window.IMGREADY_DECODE) return Promise.resolve(window.IMGREADY_DECODE);
  if (!_exoticDecoderPromise) {
    _exoticDecoderPromise = new Promise((resolve, reject) => {
      const sc = document.createElement('script');
      sc.src = '/decoders-exotic.js';
      sc.onload = () => resolve(window.IMGREADY_DECODE);
      sc.onerror = () => { _exoticDecoderPromise = null; reject(new Error('decoder load failed')); };
      document.head.appendChild(sc);
    });
  }
  return _exoticDecoderPromise;
}
function _isExoticName(file){
  return /\.(tiff?|svg|ico)$/i.test(file.name) ||
...
async function extractVideoFrames(file, onProgress){
  ...
  try {
    ...
    let dur = v.duration, vw = v.videoWidth, vh = v.videoHeight;
    if (!vw || !vh) throw new Error('That file has no video track.');
    /* duration can be Infinity (MediaRecorder output and other streamed
       files carry no duration) or NaN. Infinity happens to survive the
       maths below because n is capped, but NaN makes `wanted` NaN, which
       makes n NaN, which makes the loop run zero times and hand the
       caller an empty bufs — it then reads bufs[0] and throws something
       unreadable. Treat both as "unknown length" and let the frame cap
       decide how much we take. */
    if (!Number.isFinite(dur) || dur <= 0) dur = VIDEO_MAX_FRAMES / VIDEO_FPS;
    ...
    const bufs = [], delaysUs = [];
    for (let i = 0; i < n; i++) {
      /* Every seek needs its own deadline. The 30s guard above only
         covers loadedmetadata; nothing covered the seeks, so a file that
         opens fine but can't seek — an unseekable stream, a missing or
         damaged index — left this promise pending forever. The page kept
         painting, so it didn't look crashed: the "Reading video…" toast
         just sat there at its 60s timeout with no error and no way out.
         Bail with a readable message instead, and keep whatever frames
         we already have if it's enough to be worth something. */
      const seekOk = await new Promise((res) => {
        let settled = false;
        const done = (ok) => { if (settled) return; settled = true; clearTimeout(t); v.removeEventListener('seeked', on); res(ok); };
        const on = () => done(true);
        /* Generous on purpose. A seek into a large 4K source can take a
           second or two on a slow machine, and a false timeout here
           would truncate a clip that was going to work. This only has
           to beat "never", not be tight. */
        const t = setTimeout(() => done(false), 10000);
        v.addEventListener('seeked', on);
        try { v.currentTime = Math.min(dur - 0.001, i * step); }
        catch (_) { done(false); }
      });
      if (!seekOk) {
    ...
    return {
      ...
      duration: dur,
      /* bufs.length, not n — a seek that times out breaks out early, and
         reporting the frames we meant to take would overstate the clip
         the user is actually getting. */
      usedSeconds: bufs.length * step,
      truncated: wanted > bufs.length,
    };
  } finally {
...
async function addFilesFromList(fileList){
  ...
  for (let i = 0; i < fileList.length; i++) {
    ...
    /* Exotic formats — browsers can't render the raw blob, so
       ...
       R145: TIFF/SVG/ICO used to "fall through to a raw URL (broken paint)",
       which meant dropping a TIFF on the HOMEPAGE showed a blank thumbnail
       with no error — while every landing page decoded it correctly, because
       only they load app.js. Measured: homepage preview naturalWidth 0 vs 24
       on /tiff-to-jpg/ for the same file. Now the homepage lazy-loads
       /decoders-exotic.js (generated from src/02-decoders.js at build time,
       so there is one source of truth) the first time such a file appears. */
    ...
    if (isHeicEntry) {
      ...
      /* R145 — decode TIFF/SVG/ICO to a PNG blob purely for the PREVIEW.
         f.file stays the original so the Before meta still reads the true
         input format/size and the worker still receives what it expects. */
      try {
        const dec = await _ensureExoticDecoders();
        const png = dec && await dec.preDecodeExotic(file, 2048);
        if (png) {
          entryUrl = URL.createObjectURL(png);
          decodedBlob = png;
          try {
            const bm = await createImageBitmap(png);
            entryDims = { w: bm.width, h: bm.height };
            bm.close && bm.close();
          } catch(_) {}
        } else {
          entryUrl = URL.createObjectURL(file);
        ...
        console.warn('[exotic-decode] failed for', file.name, err);
        entryUrl = URL.createObjectURL(file);
      }
    } else {
      entryUrl = URL.createObjectURL(file);
...
/* toUpperCase() renders "WEBP" and "JPEG", but every other surface on
   this site writes "WebP" and "JPG". One label map so the working
   screen matches the marketing copy. */
const FMT_LABEL = { webp:'WebP', avif:'AVIF', jpeg:'JPG', jpg:'JPG', png:'PNG',
                    gif:'GIF', ico:'ICO', heic:'HEIC', heif:'HEIF', tiff:'TIFF',
                    bmp:'BMP', svg:'SVG', 'svg+xml':'SVG' };
function fmtLabel(x){
  const k = String(x || '').toLowerCase().replace(/^image\//, '');
  return FMT_LABEL[k] || k.toUpperCase();
}
function flFillMeta(el, f, enc){
  const inFmt = fmtLabel(f.file.type.split('/')[1] || (f.name.split('.').pop() || ''));
  const outFmt = enc ? fmtLabel(enc.format) : '';
  /* The middle dot is rationed to one per line (design skill 9.F), and
     this line used two. It also printed the format twice when it had
     not changed: "848.3 KB - PNG -> 167.4 KB - PNG". So the format is
     named once at the end when it stays the same, and only spelled out
     on both sides when the conversion actually changes it. */
  const same = !!enc && outFmt === inFmt;
  el.textContent = '';
  const before = document.createElement('span');
  before.className = 'fl-before';
  before.textContent = same
    ? `${fmtSize(f.file.size)} → `
    : `${fmtSize(f.file.size)} ${inFmt} → `;
  const after = document.createElement('span');
  after.className = 'fl-after';
  after.textContent = !enc ? 'encoding…'
    : same ? `${fmtSize(enc.size)} · ${outFmt}`
           : `${fmtSize(enc.size)} ${outFmt}`;
...
function syncCompareBar(){
  ...
  if (stats) {
    ...
    if (!f) { stats.textContent = ''; }
      ...
      const outFmt = fmtLabel(enc.format);
      const same = outFmt === inFmt;            /* see flFillMeta: one dot per line */
      stats.textContent = '';
      const before = document.createElement('span');
...
function getActiveSettings(){
  ...
  return {
    ...
    settings: {
      ...
      /* R143 — exact output dimensions (?w=&h=), overrides maxDim/resizePct */
      exactW: (window.__IMGREADY_EXACT && window.__IMGREADY_EXACT.w) || 0,
      exactH: (window.__IMGREADY_EXACT && window.__IMGREADY_EXACT.h) || 0,
    },
  };
}
...
(function initOutputPreselect(){
  ...
  /* R143 — exact dimensions from ?w=&h= */
  const _qw = parseInt(qs.get('w') || '', 10) || 0;
  const _qh = parseInt(qs.get('h') || '', 10) || 0;
  if (_qw > 0 && _qh > 0 && _qw <= 20000 && _qh <= 20000) window.__IMGREADY_EXACT = { w: _qw, h: _qh };
