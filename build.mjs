...
async function buildOnce() {
  ...
  /* R145 — Emit the exotic-format decoders as a standalone module.
     The homepage runs only inline scripts (it never loads app.js), so it had
     NO TIFF/SVG/ICO decoder and showed a broken preview for those files —
     while every landing page decoded them correctly. Rather than hand-copy a
     second implementation (which is how the two apps drifted apart in the
     first place) we EXTRACT the functions from src/02-decoders.js at build
     time, so there is exactly one source of truth.
     The extraction is validated below: if a required function is missing or
     the result does not parse, the build FAILS rather than shipping a
     silently-broken decoder. */
  {
    const NEEDED = ['getFileExt','isHeic','isTiff','isSvg','isIco',
                    'canvasToBlob','decodeLZW','parseTiff','decodeTiff',
                    'decodeSvg','decodeICO'];
    const src = readFileSync('src/02-decoders.js', 'utf8');
    const grabbed = [];
    const missing = [];
    for (const name of NEEDED) {
      /* Functions may be declared `function f(` or `async function f(`. */
      ...
      const ends = [a, b].filter(x => x !== -1);
      const nextFn = ends.length ? Math.min.apply(null, ends) : -1;
      grabbed.push(src.slice(start, nextFn === -1 ? src.length : nextFn).trimEnd());
    }
    if (missing.length) {
      throw new Error('[imgready build] decoders-exotic extraction missing: ' + missing.join(', '));
    }
    const out =
      ...
      + '})();\n';
    writeFileSync('dist/decoders-exotic.js', out);
    /* Validate: must parse as a script. */
    try { new Function(out); }
    catch (e) { throw new Error('[imgready build] decoders-exotic.js does not parse: ' + e.message); }
    console.log('[imgready build] decoders-exotic.js (' + NEEDED.length + ' fns, '
      + statSync('dist/decoders-exotic.js').size + 'B)');
  }
